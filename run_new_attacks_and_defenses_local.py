"""Tool which runs attacks and defenses in a exsiting folder and computes results."""
...
def parse_args():
  ...
  parser = argparse.ArgumentParser(
      description='Tool to selectively run attack or defenses in a exsiting folder.')
  parser.add_argument('--root_dir', required=True,
                      help='Location of everything.')
...
class Defense(Submission):
  ...
  def run(self, input_dir, output_dir):
    ...
    cmd = ['./' + self.entry_point,
           '{0}'.format(input_dir),
           '{0}'.format(output_dir) + '/result.csv']
    print(' '.join(cmd))
    subprocess.call(cmd, cwd='{0}'.format(self.directory))


def read_submissions_from_directory(dirname, result_dir, use_gpu):
  ...
  # Compare with result dire to get the new added attacks/defenses
  new_list = [ii for ii in os.listdir(dirname) if ii not in os.listdir(result_dir)]
  new_result = []
  for sub_dir in new_list:
    submission_path = os.path.join(dirname, sub_dir)
    try:
      if not os.path.isdir(submission_path):
        continue
...
def main():
  args = parse_args()
  intermediate_results_dir = os.path.join(args.root_dir, 'intermediate_results')

  attacks_output_dir = os.path.join(intermediate_results_dir,
                                    'attacks_output')
  targeted_attacks_output_dir = os.path.join(intermediate_results_dir,
                                             'targeted_attacks_output')
  defenses_output_dir = os.path.join(intermediate_results_dir,
                                     'defenses_output')
  all_adv_examples_dir = os.path.join(intermediate_results_dir,
                                      'all_adv_examples')
  print(all_adv_examples_dir)

  # Load dataset metadata.
  dataset_metadata = args.root_dir + '/dataset.csv'
  dataset_meta = DatasetMetadata(dataset_metadata)

  attacks_dir = os.path.join(args.root_dir, 'attacks')
  targeted_attacks_dir = os.path.join(args.root_dir, 'targeted_attacks')
  defenses_dir = os.path.join(args.root_dir, 'defenses')
  # Load attacks and defenses.
  a1, a2 = read_submissions_from_directory(attacks_dir, attacks_output_dir,
                                                 args.use_gpu) 
  attacks = [
      a for a in a1 if isinstance(a, Attack)
  ]
  new_attacks = [
      a for a in a2 if isinstance(a, Attack)
  ]

  a1, a2 = read_submissions_from_directory(targeted_attacks_dir, targeted_attacks_output_dir,
                                                 args.use_gpu)
  targeted_attacks = [
      a for a in a1 if isinstance(a, Attack)
  ]
  new_targeted_attacks = [
      a for a in a2 if isinstance(a, Attack)
  ]
 
  d1, d2 = read_submissions_from_directory(defenses_dir, defenses_output_dir,
                                                 args.use_gpu)
  defenses = [
      d for d in d1 if isinstance(d, Defense)
  ]
  new_defenses = [
      d for d in d2 if isinstance(d, Defense)
  ]
  print('Found attacks: ', [a.name for a in attacks])
  print('Found new attacks: ', [a.name for a in new_attacks])
  print('Found tageted attacks: ', [a.name for a in targeted_attacks])
  print('Found new tageted attacks: ', [a.name for a in new_targeted_attacks])
  print('Found defenses: ', [d.name for d in defenses])
  print('Found new defenses: ', [d.name for d in new_defenses])

  # prepare folders for new attacks and defenses
  for a in new_attacks:
    os.mkdir(os.path.join(attacks_output_dir, a.name))
  for a in new_targeted_attacks:
  ...
  # Run all new non-targeted attacks.
  dataset_dir = args.root_dir + '/dataset'
  attacks_output = AttacksOutput(dataset_dir,
                                 attacks_output_dir,
                                 targeted_attacks_output_dir,
                                 all_adv_examples_dir,
                                 args.epsilon)
  ...
  # Run all new targeted attacks.
  dataset_meta.save_target_classes(os.path.join(dataset_dir,
                                                'target_class.csv'))
  for a in new_targeted_attacks:
    a.run(dataset_dir,
  ...
  # Load all attack outputs for analysis
  for a in attacks:
  ...
  defenses_output = {}
  if (new_attacks or new_targeted_attacks):
    # either one of them is not empty; there are new attacks, run all defense
    for d in defenses:
      d.run(all_adv_examples_dir, os.path.join(defenses_output_dir, d.name))
  else:
    #both is empty; no new attacks; only run new defenses
    for d in new_defenses:
      d.run(all_adv_examples_dir, os.path.join(defenses_output_dir, d.name))
  
  # load all defense output for analysis
  for d in defenses:
