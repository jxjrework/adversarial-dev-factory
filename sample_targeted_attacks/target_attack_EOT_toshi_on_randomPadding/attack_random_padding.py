...
tf.flags.DEFINE_integer(
    'num_iter', 180, 'Number of iterations.')
...
tf.flags.DEFINE_integer(
    'ensemble_size', 30, 'how many times to tile the input tensor')

FLAGS = tf.flags.FLAGS

DEBUG = FLAGS.debug == 1
ENSEMBLE_SIZE = FLAGS.ensemble_size  # larger ensemble size -> better
...
def main(_):
    ...
    with tf.Graph().as_default():
        ...
        ensemble_xs = tf.concat([x_adv_padded for _ in range(ENSEMBLE_SIZE)], axis=0)
        ...
        ensemble_cross_entropy += tf.losses.softmax_cross_entropy(ensemble_labels, ensemble_end_points_cs3['AuxLogits'], label_smoothing=0.1, weights=0.4)
