...
def main(_):
  ...
  with tf.Graph().as_default():
    ...
    model = IrNetModel(num_classes, scope='sc')
    ...
    model_vars = [k for k in all_vars if k.name.startswith('sc')]

    # name of variable `my_var:0` corresponds `my_var` for loader
    model_keys = [s.name.replace('sc', 'InceptionResnetV2')[:-2] for s in model_vars]
